var FootnotesShs = (function () {
    function FootnotesShs() {
        ...
        this.partOfNoteIdsNotIn = ['_ftnref'];
        this.footnotes = {};
        this.hideDist = 100;
        this.startHide = 20;
    ...
    FootnotesShs.prototype.detect = function () {
        ...
        this.partsOfNoteIds.forEach(function (part) {
            var anchors = document.querySelectorAll('[id*="' + part + '"],[name*="' + part + '"]');
            var _loop_1 = function (key) {
                var id = anchors[key].id || anchors[key].getAttribute('name');
                var itsNotFootnote = that.partOfNoteIdsNotIn.some(function (part) {
    ...
    FootnotesShs.prototype.isFirstParagraphOfNote = function (element) {
        ...
        this.partsOfNoteIds.forEach(function (part) {
            ...
            if (element.children[0].getAttribute('name') &&
                element.children[0].getAttribute('name').indexOf(part) !== -1) {
                isFirst = true;
            }
        });
        return isFirst;
